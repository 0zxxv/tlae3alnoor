...
// Create student
router.post('/', (req, res) => {
  const { parent_id, name, name_ar, grade, grade_ar } = req.body;
  ...
  try {
    ...
    const id = uuidv4();
    prepare(`
      INSERT INTO students (id, parent_id, name, name_ar, grade, grade_ar) 
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(id, parent_id, name, name_ar, grade, grade_ar);
...
router.put('/:id', (req, res) => {
  const { name, name_ar, grade, grade_ar, parent_id } = req.body;
  ...
  try {
    ...
    prepare(`
      UPDATE students 
      SET name = ?, name_ar = ?, grade = ?, grade_ar = ?, parent_id = ?
      WHERE id = ?
    `).run(
      name || existing.name,
